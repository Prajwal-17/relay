...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:bg-hover hover:text-foreground focus:border-ring focus:ring-ring/50 w-full rounded px-3 py-1.5 text-left text-sm focus:ring-[3px] focus:outline-none",
        "data-selected:bg-selected data-selected:text-foreground data-selected:hover:bg-selected data-selected:hover:text-foreground",
        className
      )}
      onClick={onClick}
      onKeyDown={onKeyDown}
...
function TimePickerHour(props: TimePickerHourProps) {
  ...
  return (
    <HourPrimitive
      ...
      {...hourProps}
      className={cn(
        "flex max-h-[200px] scrollbar-none flex-col gap-1 overflow-y-auto p-1",
        className
      )}
    >
      {hours.map((hour) => (
...
function TimePickerClear(props: ButtonProps) {
  ...
  return (
    <ClearPrimitive
      ...
      {...clearProps}
      className={cn(
        "hover:bg-hover hover:text-foreground inline-flex items-center justify-center rounded-sm text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50",
        className
      )}
      onClick={onClick}
    >
